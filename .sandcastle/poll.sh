...
while true; do
  ...
  if [ "$count" -gt 0 ]; then
    ...
    if pnpm tsx .sandcastle/main.mts >>"$run_log" 2>&1; then
      ...
      # The merger merged the feature branch into local main. Push to keep
      # origin in sync. Log every branch so silent no-ops are visible.
      if git fetch origin main >>"$run_log" 2>&1; then
        ahead=$(git rev-list --count origin/main..main 2>>"$run_log") || ahead=0
        if [ "${ahead:-0}" -gt 0 ]; then
          ...
          log "local main matches origin/main — nothing to push"
        fi
      else
        log "WARN: git fetch origin main failed — see ${run_log}"
      fi
    else
